...
import TagAPI.Exceptions.TagValueNotSet;
import TagAPI.Handler.Tag.*;
            ...
            switch (currentChar) {
                ...
                case '>': //Closing tag char.
                    ...
                    if (tagHandler.tagExists(_tag)){ //if tag exists
                        ...
                        if (foundTag instanceof InstanceTag) {
                            throw new TagValueNotSet("Tag was found but was used like a FinalTag. Instance Tag cannot be used.");
                        }else if (foundTag instanceof FinalTag){
                            FinalTag tag = (FinalTag) foundTag;
                            if (tag.isDataSet()){
                                formattedString += tag.getValue();
                                _tag = "";
                            }else throw new TagValueNotSet("Tag has no value, therefore, it cannot be used.");
                        }
            ...
            switch (currentChar) {
                ...
                case '>': //Closing tag char.
                    ...
                    if (tagHandler.tagExists(_tag)){ //if tag exists
                        ...
                        if (foundTag instanceof InstanceTag) {
                            InstanceTag tag = (InstanceTag) foundTag;

                            try { //Find $Instance method to get data
                                Method data = null;
                                ...
                                if(data.getAnnotation(TagData.class) != null){
                                    ...
                                    if (dataRaw.getClass().equals(((ParameterizedType) foundTag.getClass().getGenericSuperclass()).getActualTypeArguments()[0])){ //Check if the data retrieved is of the correct type
                                        Object dataObject = tag.getValue(dataRaw);
